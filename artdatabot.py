...
class ArtDataBot:
    ...
    def updateArtworkItem(self, artworkItem, metadata):
        ...
        # Add instance (type) of artwork, can be more than one e.g. Aquatint, Print
        self.addInstances(artworkItem, metadata, metadata.get(u'refurl'))   #RML
        ...
        if metadata.get('labels'):
            ...
            if labelschanged:
                ...
                try:
                    ...
                    pass
    # Start RML
    def addInstances(self, item, metadata, url):
        """
        Add a list of instanceof statements
        """

        # Skip if we already have instanceof claims
        claims = item.get().get('claims')
        if u'P31' in claims:
            return

        # Add the instances
        for qid in metadata['instanceofqid']:
            ...
            pywikibot.output(u'Adding P31->%s to %s' % (qid, item))
    ...
    def addInception(self, item, metadata):
        ...
        if metadata.get(u'inception'):
            if type(metadata[u'inception']) is int or (len(metadata[u'inception'])==4 and \
                ...
                # Start RML
                if metadata.get(u'inceptionafter'):
                    approxdate = pywikibot.WbTime(year=metadata[u'inception'], precision=7)
                    newclaim = pywikibot.Claim(self.repo, u'P571')
                    newclaim.setTarget(approxdate)
                    pywikibot.output('Adding approx date of creation claim to %s' % item)
                    item.addClaim(newclaim)

                    earliestdate = pywikibot.WbTime(year=metadata[u'inception'])
                    earliestqualifier = pywikibot.Claim(self.repo, u'P1319')
                    earliestqualifier.setTarget(earliestdate)
                    pywikibot.output('Adding new after qualifier claim to %s' % item)
    ...
    def addMaterialUsed(self, item, metadata):
        """
        Add a list of materials used (P186) based on the medium to the item.
        If it is some kind of paint on a surface add a 'applies to part of
        painting surface' qualifier.
        ...
        if u'P186' not in claims and metadata.get(u'medium'):
            # Get the 'painting surface' property
            painting_surface = pywikibot.ItemPage(self.repo, u'Q861259')

            # Loop round all the media we have been passed
            for medium_qid, paint_on in metadata[u'medium'].items():
                medium_page = pywikibot.ItemPage(self.repo, medium_qid)
                newclaim = pywikibot.Claim(self.repo, u'P186')
                newclaim.setTarget(medium_page)
                pywikibot.output(f'Adding new medium claim to {medium_qid} {item}')
                item.addClaim(newclaim)

                # If this is a surface with some kind of paint on it then
                # add the 'applies to part of painting surface' qualifier
                if paint_on:
    ...
    def addCollectionLink(self, item, metadata):
        ...
        # Described at url
            ...
            # Start RML
            # Cope with a list of urls or just one as a string
            desc_urls = metadata.get(u'describedbyurl')
            if isinstance(desc_urls, str):
                desc_urls = [desc_urls]
            # Get all the existing described at urls
            existing = []
            if u'P973' in claims:
                for claim in claims.get(u'P973'):
                    existing.append(claim.getTarget())
            # Add the urls we don't already have
            for desc_url in desc_urls:
                if desc_url not in existing:
                    newclaim = pywikibot.Claim(self.repo, u'P973')
                    newclaim.setTarget(desc_url)
